...
namespace Graphics
{
    TextSprite::TextSprite(std::shared_ptr<TTFFont> font, std::string text) : TextSprite{Vector2{}, font, text, 10, UINT32_MAX}
    {}

    TextSprite::TextSprite(Vector2 position, std::shared_ptr<TTFFont> font, std::string text, u32 font_size, uint32_t color) :
    ...
    void TextSprite::Render()
    {
        ...
        GRRLIB_PrintfTTF(static_cast<int>(position.x), static_cast<int>(position.y), m_font->GetFont(), m_text.c_str(), m_font_size, m_color);
    }
    ...
    void TextSprite::UpdateTextSize()
    {
        ...
        u32 text_width = GRRLIB_WidthTTF(m_font->GetFont(), m_text.c_str(), m_font_size);

#include "AutofitTextSprite.h"

using namespace Graphics;

AutofitTextSprite::AutofitTextSprite(RectangleBounds bounds, std::shared_ptr<Font> font, const std::string text, uint32_t color, Vector2U32 size_range, Alignment x_align, Alignment y_align) :
...
AutofitTextSprite::AutofitTextSprite(RectangleBounds bounds, std::shared_ptr<Font> font, const std::string text, uint32_t color, Vector2U32 size_range) :
    AutofitTextSprite{bounds, font, text, color, size_range, Alignment::START, Alignment::START} {}
AutofitTextSprite::AutofitTextSprite(RectangleBounds bounds, std::shared_ptr<Font> font, const std::string text, uint32_t color) :
    AutofitTextSprite{bounds, font, text, color, Vector2U32{0, UINT32_MAX}} {}

void AutofitTextSprite::SetText(const std::string& text)
{
    this->TextSprite::SetText(text);
    this->AutofitTextSize();
}

void AutofitTextSprite::SetSize(Vector2 new_size)
{
    m_size = new_size;
    this->AutofitTextSize();
}

RectangleBounds AutofitTextSprite::GetBounds() const
{
    return RectangleBounds{this->GetPosition(), m_size};
}

void AutofitTextSprite::Render()
{
...
uint32_t AutofitTextSprite::GuessTextSize()
{
...
void AutofitTextSprite::AutofitTextSize()
{
